...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors filterNot {case (block,move) => explored contains block}
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter {case(block,moveList) => done(block)}
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    case (block,moveList) #:: tail => moveList.reverse
    case _ => List.empty
  }
}
